...
export const transferStateLink = <AppRouter extends AnyRouter>(): TRPCLink<AppRouter> => () => {
  ...
  return ({ next, op }) => {
    const shouldUseCache = op.type === 'query' &&
      !isBrowser ||  // always fetch new values on the server
    ...
    if (storeValue && isBrowser) { // on the server we don't care about the value we will always fetch a new one
