...
export default async function addThemeToApplicationGenerator(tree: Tree) {
  ...
  const themeOptions: { theme: SupportedTheme; radius: string; stylesEntryPoint?: string; prefix?: string } =
    await prompt([
      {
        type: 'select',
        required: true,
        name: 'theme',
        message: 'Choose which theme to apply:',
        hint: 'You can always re-run this generator and add a custom prefix to add other themes.',
      ...
      },
      {
        ...
        message: 'Path to the styles entry point relative to the workspace root:',
        hint: "If not provided the generator will do its best to find it and it will error if it can't.",
      },
      {
        type: 'input',
        name: 'prefix',
        message: "Prefix class name applied to your theme's style definitions:",
        hint: 'e.g., theme-rose, theme-zinc. Leave empty for global theme.',
      },
    ]);
